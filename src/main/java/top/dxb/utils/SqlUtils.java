...
import top.dxb.enums.DataTypeEnum;
...
public class SqlUtils {
	...
	public static String executeFile(String jdbcUrl, String userName, String password, String sql) {
		Connection connection = getConnection(jdbcUrl, userName, password);
		try {
			batchExecute(connection, Arrays.stream(new String[]{sql}).collect(Collectors.toList()));
		} catch (Exception e) {
			log.error("sql脚本执行发生异常:{}",e.getMessage());
			return e.getMessage();
		}finally {
